...
JNIEXPORT jboolean JNICALL
Java_com_parrot_freeflight_activities_base_ParrotCardboardActivity_onUpdateVideoTextureNative(JNIEnv *env, jobject obj, jint program, jint textureId)
{
	if (texture_initialized == FALSE) {
		init_texture();
		texture_initialized = TRUE;
	...
	if(texture.state == OPENGL_STATE_GENERATED)
	{
		...
		if (texture.data != NULL) {
//			LOGD("GL_BG_VIDEO_SPRITE", "fmt: %d, w: %f, h: %f, type: %d, data: %p", texture.format, texture.texture_size.width, texture.texture_size.height, texture.type, texture.data);
			glTexImage2D(0x8D65, 0, texture.format, texture.texture_size.width, texture.texture_size.height, 0, texture.format, texture.type, texture.data);
			texture.state = OPENGL_STATE_SEND_TO_GPU;
...
JNIEXPORT void JNICALL
Java_com_parrot_freeflight_activities_base_ParrotCardboardActivity_onSurfaceChangedNative(JNIEnv *env, jobject obj, jint width, jint height)
{
	screen_size.width = width;
	screen_size.height = height;
...
JNIEXPORT jboolean JNICALL
Java_com_parrot_freeflight_activities_base_ParrotCardboardActivity_getVideoFrameNative(JNIEnv *env, jobject obj, jobject bitmap, jfloatArray videoSize)
{
	AndroidBitmapInfo  info;
	void*              pixels;
	int                ret;
